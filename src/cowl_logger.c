...
static cowl_ret cowl_logger_open(CowlLogger *logger) {
...
static cowl_ret cowl_logger_close(CowlLogger *logger) {
    cowl_ret ret = COWL_OK;
...
static cowl_ret cowl_logger_free(CowlLogger *logger) {
...
static cowl_ret cowl_logger_log_ontology_header(CowlLogger *logger, CowlOntology *onto) {
    cowl_ret ret;
...
cowl_ret cowl_logger_clear(CowlLogger *logger) {
    if (logger->type == COWL_LT_FILE) {
        cowl_ret ret = cowl_logger_close(logger);
        if (ret) return ret;
...
cowl_ret cowl_logger_logs(CowlLogger *logger, char const *cstring) {
    cowl_ret ret = cowl_logger_open(logger);
    if (ret) return ret;
...
cowl_ret cowl_logger_consume(CowlLogger *logger, CowlString *string) {
    cowl_ret ret = cowl_logger_log(logger, string);
