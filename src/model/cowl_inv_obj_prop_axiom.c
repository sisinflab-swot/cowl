...
bool cowl_inv_obj_prop_axiom_iterate(CowlInvObjPropAxiom *axiom, CowlIterator *iter) {
    if (cowl_obj_prop_exp_iterate(axiom->first, iter) &&
        cowl_obj_prop_exp_iterate(axiom->second, iter) &&
