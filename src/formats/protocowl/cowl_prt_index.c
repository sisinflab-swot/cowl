...
static inline void prefix_map_sync_unencoded(CowlPRTPrefixMap *map) {
    ...
    uvec_foreach (CowlObjectPtr, &unencoded, e) {
        ...
        uhmap_set_val(CowlObjectPtr, &map->_ns_to_idx, key, uintptr_to_void(idx));
    }
...
prefix_map_add_encoding(CowlPRTPrefixMap *map, CowlString *ns, CowlString *prefix) {
    ...
    if (prefix) {
        ...
        if (ulib_is_err(ret)) return cowl_ret_from_ulib(ret);
        uhmap_set_val(CowlObjectPtr, &map->_ns_to_prefix, prefix_idx, cowl_retain(prefix));
    }
...
static cowl_ret prefix_map_init(CowlPRTPrefixMap *map, bool encode) {
    *map = (CowlPRTPrefixMap){
        ._idx_to_ns = uvec(CowlObjectPtr),
        ._ns_to_idx = uhmap(CowlObjectTable),
        ._ns_to_prefix = uhmap(CowlObjectTable),
        ._encode_idx = encode ? 0 : ULIB_UINT_MAX,
    };
    ...
    if (encode) prefix_map_sync_unencoded(map);
    return COWL_OK;
}
...
static CowlString *prefix_map_get(CowlPRTPrefixMap const *map, ulib_uint idx) {
    if (!uvec_index_is_valid(CowlObjectPtr, &map->_idx_to_ns, idx)) return NULL;
...
static void id_map_init(CowlPRTIdMap *map, bool encode) {
    *map = (CowlPRTIdMap){
        ._idx_to_id = uvec(CowlObjectPtr),
        ._id_to_idx = uhmap(CowlObjectTable),
        ._encode_idx = encode ? 0 : ULIB_UINT_MAX,
    };
}
...
static void id_map_sync_unencoded(CowlPRTIdMap *map) {
    ...
    uvec_foreach (CowlObjectPtr, &unencoded, e) {
        ...
        ulib_uint const idx = map->_encode_idx + e.i;
        uhmap_set_val(CowlObjectPtr, &map->_id_to_idx, key, uintptr_to_void(idx));
    }
...
static CowlIRIOrAnonInd *id_map_get(CowlPRTIdMap *map, ulib_uint idx, cowl_ret *ret) {
    ulib_uint const count = uvec_count(CowlObjectPtr, &map->_idx_to_id);
...
CowlIRIOrAnonInd *cowl_prt_index_get_id(CowlPRTIndex *index, ulib_uint idx, cowl_ret *ret) {
    return id_map_get(&index->_imap, idx, ret);
}
